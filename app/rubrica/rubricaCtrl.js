...
(function () {
	'use strict';
	var controllerId = 'rubricaCtrl';
	angular.module('app')
				 .controller(controllerId, ['dataList', 'dataFactory', '$stateParams', '$state', '$scope', '$filter', '$uibModal', 'toastr', rubricaCtrl]);


	function rubricaCtrl(dataList, dataFactory, $stateParams, $state, $scope, $filter, $modal, toastr) {
		var vm = this;
		vm.title = 'Rubrica';
		vm.subtitle = 'Aziende e Produttori';
		vm.collection = 'contatti';
    ...
    vm.setFilter = setFilter;
		...
		function getList() {
			if(!vm.filter.search && !vm.filter.tipo) {
				...
				var filter = "";
				if(vm.filter.search) {
					filter += "ragioneSociale=" + vm.filter.search + "&";
				}

				if(vm.filter.tipo) {
					filter += "tipo=" + vm.filter.tipo + "&";
				}
